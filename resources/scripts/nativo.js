window.addEventListener('DOMContentLoaded', () => {
  ...
  const { arcSite, globalContentConfig, layout } = window.Fusion;
  ...
  const isStory = layout === 'article-basic' || layout === 'article-in-depth';
  ...
  if ((isStory || isLiveUpdatesPage) && arcSite !== 'dayton') {
    ...
    if ((meterMeta.getAttribute('content') === 'premium' || meterMeta.getAttribute('content') === 'subscriberonly') && allowMeter) {
