...
const filterContentElements = ({ contentElements, elementToRemove = 'divider', indexToStartRemoving = 1 }) => {
  const newContentElements = contentElements.filter((element) => {
    ...
    return (
      (
        element.type !== elementToRemove // it's not a removable content type, OR
        || (
    ...
    ) ? element : null; // then include the content element in the (new) array of elements to be passed back for rendering
  });
